...
class Metric(object):
    def __init__(self, dim=2048):
        self.inception = InceptionV3([InceptionV3.BLOCK_INDEX_BY_DIM[dim]])
        self.cifar10_mu = None
        self.cifar10_sigma = None

    def cifar10_FID(self, dataloader, gpu_ids):
        self.cifar10_mu = pd.read_csv("Cifar_10_M.csv").values
        self.cifar10_sigma = pd.read_csv("Cifar_10_S.csv").values
        fake_mu, fake_sigma = self.compute_act_statistics(dataloader, self.inception, gpu_ids)
        fid = self.FID(self.cifar10_mu, self.cifar10_sigma, fake_mu, fake_sigma)
        return fid
    ...
    #     metric = Metric()
    #     m1, s1 = metric.compute_act_statistics_from_loader(loader, _InceptionV3([_InceptionV3.BLOCK_INDEX_BY_DIM[2048]]), [])
